...
save_dir = os.path.join(data_dir, "examples/images")
if not os.path.exists(save_dir):
    os.makedirs(save_dir)
    
save_frame_dir = os.path.join(data_dir, "examples/frames")
...
with alive_bar(len(video_id_list)) as bar:
    ...
    for video_path_name in video_id_list:
        ...
        track_path = os.path.join(data_dir, "tracks", f"{videoid}.txt")
        ...
        while True:
            ...
            if frameid not in track_data or frameid % frame_gap != 0 or frameid < 1000 or frameid > 2000:
                continue
            ...
            cv2.imwrite(os.path.join(save_frame_dir, f"{frameid:04d}.jpg"), frame)
        bar()