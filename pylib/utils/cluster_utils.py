...
def Postprocess(before_tracks,
    ...
    for trackid in before_tracks:
        ...
        closestCluster = -1
        bestDistance = -1
        track_frame_ids = sorted(list(before_tracks[trackid][0].keys()))
        start_frame_id, end_frame_id = track_frame_ids[0], track_frame_ids[-1]
        position_list = [[before_tracks[trackid][0][track_frame_id][0] + before_tracks[trackid][0][track_frame_id][2] / 2.0,
                                       before_tracks[trackid][0][track_frame_id][1] + before_tracks[trackid][0][track_frame_id][3] / 2.0] for track_frame_id in track_frame_ids]
        for ci, (_, cluster_track, cluster_num) in enumerate(clusters):
            track_d = TrackDistance2([[(point[0] + point[2])/2,
                                       (point[1] + point[3])/2] for point in cluster_track], 
                                     position_list)
        ...
        pnext = NextDetection(position_list, 0, 1)
        if pnext != -1 and start_frame_id > 1:
            vector1 = [(prefix[0] + prefix[2])/2 - position_list[0][0],
                       (prefix[1] + prefix[3])/2 - position_list[0][1]]
            vector2 = [position_list[0][0] - position_list[pnext][0],
                       position_list[0][1] - position_list[pnext][1]]
            angle = SignedAngle(vector1, vector2)
            if abs(angle) < math.pi / 4:
                # Find the closest point to the first point of the "position_list" in the cluster.
                _, p_n_p_i = find_nearest_point_of_track(position_list[0],
                                                         [[(point[0] + point[2])/2,
                                                           (point[1] + point[3])/2] for point in closestCluster[1]])
                start_frame_id = int(max(
                    start_frame_id - p_n_p_i, 1))
                postprocess_tracks[trackid][0][start_frame_id] = [prefix[0], prefix[1],
                                                                  int(prefix[2] - prefix[0]),
                                                                  int(prefix[3] - prefix[1]),
                                                                  1.0]

        snext = NextDetection(position_list,
                              len(position_list) - 1, -1)
        if snext != -1 and end_frame_id < maxFrame:
            vector1 = [(suffix[0] + suffix[2])/2 - position_list[-1][0],
                       (suffix[1] + suffix[3])/2 - position_list[-1][1]]
            vector2 = [position_list[-1][0] - position_list[snext][0],
                       position_list[-1][1] - position_list[snext][1]]
            angle = SignedAngle(vector1, vector2)
            if abs(angle) < math.pi / 4:
                # Find the closest point to the first point of the "position_list" in the cluster.
                _, s_n_p_i = find_nearest_point_of_track(position_list[-1],
                                                         [[(point[0] + point[2])/2,
                                                           (point[1] + point[3])/2] for point in closestCluster[1]])
                end_frame_id = int(min(end_frame_id + abs(len(closestCluster[1]) - s_n_p_i),
                                                                                 maxFrame))
                postprocess_tracks[trackid][0][end_frame_id] = [suffix[0], suffix[1],
                                                                int(suffix[2] - suffix[0]),
                                                                int(suffix[3] - suffix[1]),
                                                                1.0]
                # use_suffix = True
