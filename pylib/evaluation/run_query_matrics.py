...
for className in classes:
    Results[className] = {"recall": [], "precision": [], "accuracy": [],
                          "f1": [], "mae": [], "acc": [], "gt_count": [],
                          "pred_count": [], "acc_topk": [], "cardinality_f1": [], "cardinality2_f1": [], "cardinality3_acc": [],

                          "crecall": [], "cprecision": [], "caccuracy": [],
                          "cf1": [], "cmae": [], "cacc": [], "cgt_count": [],
                          "cpred_count": [], "cacc_topk": [], "ccardinality_f1": []}
...
while True:
    ...
    for videoid, skipframe in zip(videoids, skipframes):
        ...
        with open(trackpath, "r") as reader:
            for dataline in reader:
                if len(dataline.split(",")) != 1:
                    remapframeid, trackid, x1, y1, width, height, score, classid, _, _, _ = dataline.split(
                        ",")
                    remapframeid = int(remapframeid)
                    frameid = (remapframeid - 1) * skipframe + 1
                    trackid = int(trackid)
        ...
        for className in classes:
            ...
            recall, precision, accuracy, f1, mae, acc, gt_vehicle, pred_vehicle, acc_topk, cardinality_f1, cardinality2_f1, cardinality3_acc, \
                recall_constrain, precision_constrain, accuracy_constrain, f1_constrain, mae_constrain, \
                acc_constrain, gt_vehicle_constrain, pred_vehicle_constrain, acc_topk_constrain, cardinality_f1_constrain = QM.matrics(new_gt_tuple, new_pred_tuple,
                                                                                                                                       constrain=60)
            ...
            if cardinality_f1 is not None:
                Results[className]["cardinality_f1"].append(cardinality_f1)
            if cardinality2_f1 is not None:
                Results[className]["cardinality2_f1"].append(cardinality2_f1)
            Results[className]["cardinality3_acc"].append(cardinality3_acc)
            Results[className]["crecall"].append(recall_constrain)
            Results[className]["cprecision"].append(precision_constrain)
            Results[className]["caccuracy"].append(accuracy_constrain)
            Results[className]["cf1"].append(f1_constrain)
            Results[className]["cmae"].append(mae_constrain)
            Results[className]["cacc"].append(acc_constrain)
            Results[className]["cgt_count"].append(gt_vehicle_constrain)
            Results[className]["cpred_count"].append(pred_vehicle_constrain)
            Results[className]["cacc_topk"].append(acc_topk_constrain)
            if cardinality_f1_constrain is not None:
                Results[className]["ccardinality_f1"].append(
    ...
    for className in classes:
        if len(Results[className]["recall"]) == 0:
            ...
            Results[className]["crecall"] = None
            Results[className]["cprecision"] = None
            Results[className]["caccuracy"] = None
            Results[className]["cf1"] = None
            Results[className]["cmae"] = None
            Results[className]["cacc"] = None
            Results[className]["cgt_count"] = 0
            Results[className]["cpred_count"] = 0
            Results[className]["cacc_topk"] = None
            Results[className]["crecall_mean"] = 0
            Results[className]["cprecision_mean"] = 0
            Results[className]["caccuracy_mean"] = 0
            Results[className]["cf1_mean"] = 0
            Results[className]["cmae_mean"] = 0
            Results[className]["cacc_mean"] = 0
            Results[className]["cgt_count_mean"] = 0
            Results[className]["cpred_count_mean"] = 0
            Results[className]["cacc_topk_mean"] = 0

            continue

        if len(Results[className]["cardinality_f1"]) == 0:
            Results[className]["cardinality_f1"] = None
            Results[className]["cardinality_f1_mean"] = None
        else:
            Results[className]["cardinality_f1_mean"] = float(
                sum(Results[className]["cardinality_f1"]) / len(Results[className]["cardinality_f1"]))

        if len(Results[className]["ccardinality_f1"]) == 0:
            Results[className]["ccardinality_f1"] = None
            Results[className]["ccardinality_f1_mean"] = None
        else:
            Results[className]["ccardinality_f1_mean"] = float(
                sum(Results[className]["ccardinality_f1"]) / len(Results[className]["ccardinality_f1"]))

        if len(Results[className]["cardinality2_f1"]) == 0:
            Results[className]["cardinality2_f1"] = None
            Results[className]["cardinality2_f1_mean"] = None
        else:
            Results[className]["cardinality2_f1_mean"] = float(
                sum(Results[className]["cardinality2_f1"]) / len(Results[className]["cardinality2_f1"]))

        if len(Results[className]["cardinality3_acc"]) == 0:
            Results[className]["cardinality3_acc"] = None
            Results[className]["cardinality3_acc_mean"] = None
        else:
            Results[className]["cardinality3_acc_mean"] = float(
                sum(Results[className]["cardinality3_acc"]) / len(Results[className]["cardinality3_acc"]))
        ...
        Results[className]["crecall_mean"] = float(
            sum(Results[className]["crecall"]) / len(Results[className]["crecall"]))
        Results[className]["cprecision_mean"] = float(
            sum(Results[className]["cprecision"]) / len(Results[className]["cprecision"]))
        Results[className]["caccuracy_mean"] = float(
            sum(Results[className]["caccuracy"]) / len(Results[className]["caccuracy"]))
        Results[className]["cf1_mean"] = float(
            sum(Results[className]["cf1"]) / len(Results[className]["cf1"]))
        Results[className]["cmae_mean"] = float(
            sum(Results[className]["cmae"]) / len(Results[className]["cmae"]))
        Results[className]["cacc_mean"] = float(
            sum(Results[className]["cacc"]) / len(Results[className]["cacc"]))
        Results[className]["cgt_count_mean"] = int(
            sum(Results[className]["cgt_count"]))
        Results[className]["cpred_count_mean"] = int(
            sum(Results[className]["cpred_count"]))
        Results[className]["cacc_topk_mean"] = float(
            sum(Results[className]["cacc_topk"]) / len(Results[className]["cacc_topk"]))


    # print(json.dumps(Results, indent=4, sort_keys=True))
    pickle.dump(Results, open(os.path.join(
        f"./result/{method_name}/{dataname}", "query_matrics.pkl"), "wb"))
