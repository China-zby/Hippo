...
class QueryMatrics(object):
    ...
    def cardinality_limited_query_1(self):
        ...
        if max_object_num > 1:
            cardinality_object_num = max(
                object_num_list, key=lambda x: object_num_list.count(x))
        else:
            return None
    ...
    def cardinality_limited_query_2(self, input_label, input_pred):
        if self.object_type == 2:
            ...
            bus_object_num_list = list(bus_gt_result.values())
            bus_num = max(bus_object_num_list,
                          key=lambda x: bus_object_num_list.count(x))
            car_object_num_list = list(self.gt_result.values())
            car_object_num_list_constrain = [car_object_num_list[pos] for pos in range(
                len(bus_object_num_list)) if bus_object_num_list[pos] == bus_num]
            car_num = max(car_object_num_list_constrain)

            meet_the_condition_gt_frame_id = []
            meet_the_condition_pred_frame_id = []
            for frame_id in range(1, self.end_frame):
                # print("self.gt_result[frame_id], car_num, bus_gt_result[frame_id], bus_num: ", self.gt_result[frame_id], car_num, bus_gt_result[frame_id], bus_num, file=open("cardinality.txt", "a"))
    ...
    def cardinality_limited_query_3_old(self, input_label, input_pred):
        # if self.object_type != 2:
        #     return 1.0
        left_turn_num = 0
        right_turn_num = 0
        ...
        if left_turn_num != 0:
            total += 1
            acc += max(1 - abs(pred_left_turn_num -
                       left_turn_num) / left_turn_num, 0)
        if right_turn_num != 0:
            total += 1
            acc += max(1 - abs(pred_right_turn_num -
                               right_turn_num) / right_turn_num, 0)
        if straight_num != 0:
            total += 1
            acc += max(1 - abs(pred_straight_num -
                       straight_num) / straight_num, 0)
        if total == 0:
            return 0
        acc = acc / total
        return acc
    
    def cardinality_limited_query_3(self, input_label, input_pred, constrain=None):
        gt_turning_left_keys = []
        for key in input_label.keys():
            if input_label[key]["class_id"] == self.object_type:
                trajectory = input_label[key]["position_list"]
                trajectory_class = self.classify_trajectory(trajectory)
                if trajectory_class == 1:
                    gt_turning_left_keys.append(key)

        pred_turning_left_keys = []
        for key in input_pred.keys():
            if input_pred[key]["class_id"] == self.object_type:
                trajectory = input_pred[key]["position_list"]
                trajectory_class = self.classify_trajectory(trajectory)
                if trajectory_class == 1:
                    pred_turning_left_keys.append(key)

        pred_frame_result = {i: 0 for i in range(1, self.end_frame)}
        gt_frame_result = {i: 0 for i in range(1, self.end_frame)}
        for key in gt_turning_left_keys:
            if input_label[key]["frame_bound"]["start_frame_id"] < self.end_frame:
                if abs(input_label[key]["frame_bound"]["end_frame_id"] - input_label[key]["frame_bound"]["start_frame_id"]) >= constrain:
                    for i in range(input_label[key]["frame_bound"]["start_frame_id"],
                                    min(input_label[key]["frame_bound"]["end_frame_id"] + 1, self.end_frame)):
        ...
        for key in pred_turning_left_keys:
            if input_pred[key]["frame_bound"]["start_frame_id"] < self.end_frame:
                if abs(input_pred[key]["frame_bound"]["end_frame_id"] - input_pred[key]["frame_bound"]["start_frame_id"]) >= constrain:
                    for i in range(input_pred[key]["frame_bound"]["start_frame_id"],
                                    min(input_pred[key]["frame_bound"]["end_frame_id"] + 1, self.end_frame)):
                        pred_frame_result[i] += 1
                        
        TP, FP, TN, FN = 0, 0, 0, 0

        for frame_id in range(1, self.end_frame):
            if gt_frame_result[frame_id] >= 1 and pred_frame_result[frame_id] >= 1:
                TP += 1
            elif gt_frame_result[frame_id] >= 1 and pred_frame_result[frame_id] == 0:
                FN += 1
            elif gt_frame_result[frame_id] == 0 and pred_frame_result[frame_id] >= 1:
                FP += 1
            else:
                TN += 1
        if TP == 0 and FP == 0:
        ...
        # 保存结果到文件
        # with open("/home/lzp/go-work/src/videotune/pylib/evaluation/tp_fn_results.txt", "a") as file:
        #     file.write(f"TP: {TP}, FN: {FN}\n")
    ...
    def matrics(self, gt_tuple, pred_tuple, constrain=None):
        ...
        cardinality2_f1 = self.cardinality_limited_query_2(
            gt_tuple, pred_tuple)
        cardinality3_acc = self.cardinality_limited_query_3(
            gt_tuple, pred_tuple, constrain=constrain)
