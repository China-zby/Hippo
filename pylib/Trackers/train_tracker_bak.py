...
parses.add_argument('--seed', type=int, default=10086,
...
for epoch in range(9999):
    ...
    with alive_bar(512) as trainbar:
        for _ in range(512):
            ...
            sceneids = torch.LongTensor(np.asarray(
                [example[4] for example in examples])).cuda()
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(f"./figures/train_tracker_loss_bak_{skips[-1]}.jpg",
    ...
    if best_loss == 9999 or val_loss < best_loss:
        ...
        torch.save(model.state_dict(), os.path.join(os.path.join(
            args.save_root, "Trackers", args.data_name), 'IOU_bak_{}.pth'.format(skips[-1])))
        bad_loss_streak = 0
    else:
        bad_loss_streak += 1
