...
def make_window_by_mode(mode):
    candidate_windows = []
    AllBounds = [[640,352],[416,256],[320,192],[192,64],[64,32]]
    mode = str(mode)
    window_number = int(mode[0])
    Bound_ids = [int(i) for i in mode[1:]]
    for i in range(len(Bound_ids)):
        for j in range(Bound_ids[i]):
            candidate_windows.append(AllBounds[i])
    
    candidate_bboxes = []
    for candidate_i, candidate_window in enumerate(candidate_windows):
        if candidate_i == 0:
            start_y = random.randint(0, 352 - candidate_window[1])
            candidate_bbox = [0, start_y, candidate_window[0], start_y+candidate_window[1]]
            start_x = candidate_window[0]
        else:
            start_y = random.randint(0, 352 - candidate_window[1])
            candidate_bbox = [start_x, start_y, start_x+candidate_window[0], start_y+candidate_window[1]]
            start_x += candidate_window[0]
        candidate_bboxes.append(candidate_bbox)
    return candidate_bboxes

def load_target(fname, clses, input_dim, orig_dim, lenient=False):
	def input_clip(x, y):
...
def get_windows_from_bin(bin, sizes=None):
	bin = numpy.copy(bin)
	if bin.dtype != 'bool':
