...
func main() {
	...
	for _, roimodelsize = range roimodelsizes {
		best_f1, best_threshold, descent_round := 0.0, -1.0, 0
		xs, ys := make([]float64, 0), make([]float64, 0)
		cfg.RoiBase.Flag = true
		cfg.VideoBase.SkipNumber = 32
		cfg.RoiBase.Resolution = roimodelsize
		for _, threshold := range thresholds {
			cfg.RoiBase.Threshold = threshold
			f1 := lib.RoiPart(cfg, threadCount)
			if f1 >= best_f1 {
				best_f1 = f1
		...
		p.Title.Text = "Find the best threshold for " + fmt.Sprintf("%dx%d", roimodelsize[0], roimodelsize[1])
		...
		savePath := filepath.Join(saveRoot, fmt.Sprintf("%dx%d", roimodelsize[0], roimodelsize[1])+"_threshold.png") // saveRoot + fmt.Sprintf("%dx%d", roimodelsize[0], roimodelsize[1]) + "_threshold.png"
		...
		fmt.Printf("Line graph saved to '%s'.", fmt.Sprintf("%dx%d", roimodelsize[0], roimodelsize[1])+"_threshold.png")
		FindResult[fmt.Sprintf("%d_%d", roimodelsize[0], roimodelsize[1])] = best_threshold
	}
