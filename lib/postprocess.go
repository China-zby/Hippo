...
func SampleNormalizedPoints(track []TrackDetection) []common.Point {
	...
	remaining := pointFreq
	for i := 0; i < len(track)-1; i++ {
		segment := common.Segment{Start: track[i].Rectangle().Center(), End: track[i+1].Rectangle().Center()}
		for segment.Length() > remaining {
			vector := segment.Vector()
			p := segment.Start.Add(vector.Scale(remaining / segment.Length()))
			points = append(points, p)
			segment = common.Segment{Start: p, End: segment.End}
			remaining = pointFreq
		}
...
func Postprocess(clusters []Cluster, tracks [][]TrackDetection, SkipNumber int) [][]TrackDetection {
	...
	for i, track := range tracks {
		...
		if pnext != -1 && track[0].FrameIdx > 0 {
			...
			if math.Abs(angle) < math.Pi/4 {
				prefix.FrameIdx = track[0].FrameIdx - SkipNumber
		...
		if snext != -1 && track[len(track)-1].FrameIdx < maxFrame {
			...
			angle := vector1.SignedAngle(vector2)
			if math.Abs(angle) < math.Pi/4 {
				suffix.FrameIdx = track[len(track)-1].FrameIdx + SkipNumber
...
func NewMiris(cfg JsonConfig) *Miris {
	cmd := exec.Command("python", "./pylib/Postprocess/miris.py", strconv.Itoa(cfg.DeviceID))
	cmd.Env = append(os.Environ(), "CUDA_VISIBLE_DEVICES="+strconv.Itoa(cfg.DeviceID))
...
func (miris *Miris) process(tracks [][]TrackDetection, SkipNumber int, cameraInfo CameraInfo) [][]TrackDetection {
	packet := struct {
		...
		Scene:       cameraInfo.SceneName,
		FrameNumber: cameraInfo.FrameNumber,
		Type:        "job",
	}

	miris.mu.Lock()
	defer miris.mu.Unlock()

	bytes := JsonMarshal(packet)
	header := make([]byte, 4)
	binary.BigEndian.PutUint32(header, uint32(len(bytes)))
	miris.stdin.Write(header)
	miris.stdin.Write(bytes)

	var line string
	for {
		var err error
		line, err = miris.rd.ReadString('\n')
...
func (miris *Miris) Close() {
	miris.stdin.Close()
	miris.cmd.Wait()
}

func PostprocessMIRIS(tracks [][]TrackDetection, SkipNumber int, cameraInfo CameraInfo, miris *Miris) [][]TrackDetection {
	processTracks := miris.process(tracks, SkipNumber, cameraInfo)
