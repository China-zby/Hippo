...
class ProbabilisticDistribution:
    def __init__(self, preds):
        ...
        # 创建一个Categorical分布列表以便复用，这样做是为了数值稳定性和效率
        self.distributions = [torch.distributions.Categorical(probs=pred) for pred in self.preds]
    ...
    def get_actions(self, deterministic=False):
        ...
        for bi in range(self.bs):
            if deterministic:
                ...
                action = [dist.sample().item() for dist in self.distributions]
            actions.append(action)
        actions = torch.tensor(actions, dtype=torch.long)  # 确保actions是长整型，这对索引很重要
        return actions

    def log_prob(self, actions):
        log_probs = []
        for bi, a in enumerate(actions):
            log_prob = sum(dist.log_prob(a[ai]) for ai, dist in enumerate(self.distributions))
    ...
    def cross_entropy(self, other):
        cross_entropies = [dist.cross_entropy(other[bi]) for bi, dist in enumerate(self.distributions)]
        return torch.stack(cross_entropies)
...
class DUPIN(ActorCriticPolicy):
    def __init__(self, hidden_size=64, video_context_size=16, observation_space=None,
        ...
        # self.mean = nn.Parameter(torch.zeros(hidden_size), requires_grad=False)
        # self.std = nn.Parameter(torch.ones(hidden_size), requires_grad=False)
        ...
        for batch in range(0, episodes, batch_size):
            ...
            if batch != 0:
                for _ in range(strong_update_step):
                    pareto_batch_vids, pareto_batch_idxs, \
                    pareto_batch_rewards, pareto_batch_saved_log_probs = [], [], [], []
                    for train_video_id in env.video_ids:
                        pareto_episode_log_probs, pareto_episode_rewards = [], []
                        ...
                        pareto_set_gt = sorted(pareto_set_gt, key=lambda x: x[0], reverse=True)
                        ...
                        while not done and not truncated:
                            ...
                            if len(pareto_set_gt_queue) == 0:
                                ...
                                continue
                            pareto_acc, pareto_lat, pareto_action = pareto_set_gt_queue.pop(0)
                            observation_, done, truncated = env.step_without_ingestion(pareto_acc, pareto_lat,
                                                                                       torch.tensor([pareto_action]))
                            
                            pareto_log_probs = action_distribution.log_prob(torch.tensor([pareto_action]))
                            
                            pareto_episode_log_probs.append(pareto_log_probs)
                            pareto_episode_rewards.append(1.0)
                        
                        R = 0
                        pareto_episode_returns, pareto_episode_idxs, pareto_episode_vids = [], [], []
                        for i, r in enumerate(pareto_episode_rewards[::-1]):
                            R = r + gamma * R
                            interactive_round = len(pareto_episode_rewards) - i - 1
                            pareto_episode_returns.insert(0, R)
                            pareto_episode_idxs.insert(0, interactive_round)
                            pareto_episode_vids.insert(0, env.current_video)
                        
                        pareto_batch_vids.extend(pareto_episode_vids)
                        pareto_batch_idxs.extend(pareto_episode_idxs)
                        pareto_batch_rewards.extend(pareto_episode_returns)
                        pareto_batch_saved_log_probs.extend(pareto_episode_log_probs)
                        
                    pareto_batch_saved_log_probs = torch.cat(pareto_batch_saved_log_probs)
                    policy_losses = []
                    for log_prob, R in zip(pareto_batch_saved_log_probs,
                                        pareto_batch_rewards):
...
if __name__ == "__main__":
    ...
    channel = 800
    method_name = "detective_reinforce_all_context_graph"
    data_name = "hippo"
    scene_name = "hippo"
    ...
    pretrain_weight = "./reinforce_tensorboard_all_context_graph2/eval_result/best_model_512" # "./reinforce_tensorboard_all/eval_result/best_model_8896"
    ...
    reinforce(env, test_env, policy, optimizer, policy_deviceid,
              batch_size, update_step,
