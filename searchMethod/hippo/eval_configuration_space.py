...
from utils import SEARCH_SPACE, SEARCH_SPACE_NAMES, GOLDEN_CONFIG_VECTOR, mean, \
                 OTIF_SEARCH_SPACE, OTIF_SEARCH_SPACE_NAMES, OTIF_GOLDEN_CONFIG_VECTOR, \
                 SKY_SEARCH_SPACE, SKY_SEARCH_SPACE_NAMES, SKY_GOLDEN_CONFIG_VECTOR, \
                 MED_SEARCH_SPACE, MED_SEARCH_SPACE_NAMES, MED_GOLDEN_CONFIG_VECTOR
...
class DUPIN(ActorCriticPolicy):
    def __init__(self, search_space, search_space_name, 
                 hidden_size=64, video_context_size=16, observation_space=None,
...
class HippoEnv(gym.Env):
    def __init__(self, dataset, feature_extractor_model, camera_config,
                 ...
                 search_space, search_space_name, golden_config_vector,
                 debug=False, use_init_solution=True):
        ...
        self.search_space = search_space
        self.search_space_name = search_space_name
        self.golden_config_vector = golden_config_vector
    ...
    def prepare_observation(self, pareto_set):
        processed_pareto_set = np.ones(
            (self.max_pareto_set_size, len(self.search_space) + 2)) * -1
    ...
    def step(self, action, env=None):
        ...
        if self.debug:
            ...
            _, ingvalue, _, _, _, \
                cmetric, _ = self.video_camera_config.ingestion # _without_cache
    ...
    def reset(self, video_id=None, env=None):
        ...
        if not self.use_init_solution:
            ...
            if len(self.init_solution) == 0:
                self.init_solution = [self.golden_config_vector]
            for action in self.init_solution:
                self.video_camera_config.updateConfig(utils.generate_config(action,
                                                                            self.video_camera_config.loadConfig(),self.search_space_name))
                if self.debug:
                    accuracy, latency = random.random(), random.random()
                else:
                    _, ingvalue, _, _, _, \
...
def reinforce(env, test_env,
              ...
              save_train=False, strong_update_step=10, cache_store_path=None):
    training_time = 0.0
    ...
    with Progress(*custom_columns) as progress:
        ...
        for batch in range(0, episodes, batch_size):
                ...
                for _ in range(strong_update_step):
                    ...
                    for train_video_id in env.video_ids:
                        ...
                        pareto_set_gt = env.collocate_pareto_set(
                            env.current_video, cache_dir=cache_store_path)
            ...
            for episode in range(batch_size):
                ...
                pareto_set_gt = env.collocate_pareto_set(env.current_video, cache_dir=cache_store_path)
                done, truncated = False, False
                ...
                while not done and not truncated:
                    ...
                    observation_, reward, done, truncated, info = env.step(
                        action.cpu())
                    # print("step_index: ", step_index, "reward", reward)
                    ...
                    plt.text(0.7, 0.9 - (iter_num - 1) * 0.05, f"{iter_num}: {reward:.6f}")
                    plt.xlim(0, 1)
                    plt.ylim(0, 1)
                    plt.xlabel("Accuracy")
                    plt.ylabel("Latency")
...
if __name__ == "__main__":
    ...
    search_spaces = [SKY_SEARCH_SPACE, OTIF_SEARCH_SPACE, MED_SEARCH_SPACE, SEARCH_SPACE]
    search_space_names = [SKY_SEARCH_SPACE_NAMES, OTIF_SEARCH_SPACE_NAMES, MED_SEARCH_SPACE_NAMES, SEARCH_SPACE_NAMES]
    golden_config_vectors = [SKY_GOLDEN_CONFIG_VECTOR, OTIF_GOLDEN_CONFIG_VECTOR, MED_GOLDEN_CONFIG_VECTOR, GOLDEN_CONFIG_VECTOR]
    
    # names = ["FULL"]
    # search_spaces = [SEARCH_SPACE]
    # search_space_names = [SEARCH_SPACE_NAMES]
    # golden_config_vectors = [GOLDEN_CONFIG_VECTOR]
    
    for name, search_space, search_space_name, golden_config_vector in zip(names, 
                                                                           search_spaces,
                                                                           search_space_names,
                                                                           golden_config_vectors):  
        if name != "OTIF":
            continue
        start_time = time.time()
        channel = 800
        method_name = f"search_space_{name}"
        data_name = "hippo"
        scene_name = "hippo"
        ...
        effiency_targets = []
        train_video_list = [0] # [0, 100, 200, 300, 400, 500, 600, 700]
        ...
        log_dir = f"./logs/hippo_search_space_{name}_checkpoint/"
        ...
        cache_store_path = f"./cache/global/one_camera_with_config_with_searchspace_{name}"

        if not os.path.exists(cache_store_path):
            os.makedirs(cache_store_path)
        ...
        policy = DUPIN(search_space, search_space_name, hidden_size=hidden_size, video_context_size=video_context_size,
        ...
        env = HippoEnv(train_dataset, None, train_cameras[0],
                    max_iter, max_pareto_set_size, effiency_targets, 
                    search_space, search_space_name, golden_config_vector,
                    debug=debug, use_init_solution=use_init_solution)
        ...
        reinforce(env, env, policy, optimizer, policy_deviceid,
                  ...
                  episodes=500, baseline_window_size=baseline_window_size,
                  gamma=0.99, lr_decay=lr_decay_flag,
                  video_list=train_video_list, entropy_coef=entropy_coef,
                  save_train=save_train, strong_update_step=strong_update_step,
                  cache_store_path=cache_store_path)
        train_time = time.time()-start_time
        print(f"{name} Training time: {train_time}")